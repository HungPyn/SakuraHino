...
function formatDate(dateString) {
  ...
  // Đảm bảo đây là đối tượng Date, vì từ service trả về Date objects
...
function getSortIcon(field) {
    if (props.sortByField === field) {
        // Sử dụng bi-sort-up và bi-sort-down để rõ ràng hơn về hướng
        return props.sortDirection === 'asc' ? 'bi bi-sort-up' : 'bi bi-sort-down';
    }
    return 'bi bi-sort-alpha-down'; // Icon mặc định khi chưa sắp xếp
}
</script>

<style scoped>
/* Styles from LessonManagementPage.vue for this section */
.card {
  border: none;
...
.badge {
  ...
  white-space: nowrap; /* Ngăn không cho icon và text xuống dòng */
}
