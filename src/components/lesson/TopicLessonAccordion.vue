<template>
  <div class="topic-lesson-accordion mt-4">
    ...
    <div v-else-if="topicsWithLessons.length === 0" class="alert alert-info text-center" role="alert">
      Không có chủ đề nào để hiển thị. Vui lòng thêm chủ đề mới.
    </div>

    <div v-else class="accordion" id="topicAccordion">
      <div v-for="topic in topicsWithLessons" :key="topic.id" class="accordion-item mb-3 shadow-sm rounded">
        <h2 class="accordion-header" :id="'heading' + topic.id">
          <button class="accordion-button d-flex align-items-center"
            ...
            <img :src="topic.url_image" class="topic-icon me-3" alt="Topic Icon" v-if="topic.url_image">
            <span class="topic-name flex-grow-1">{{ topic.name }} ({{ topic.lessons.length }} bài học)</span>
            <span class="topic-actions ms-auto">
              <button class="btn btn-sm btn-outline-secondary me-2" @click.stop="editTopic(topic.id)" title="Chỉnh sửa chủ đề">
                <i class="bi bi-pencil-square"></i>
              </button>
              <button class="btn btn-sm btn-outline-danger" @click.stop="deleteTopic(topic.id)" title="Xóa chủ đề">
        ...
        <div :id="'collapse' + topic.id" class="accordion-collapse collapse"
             ...
             :data-bs-parent="isTopicView ? '#topicAccordion' : ''">
          <div class="accordion-body">
            <p class="text-muted small mb-3">{{ topic.description || 'Chủ đề này chưa có mô tả.' }}</p>
            <div v-if="topic.lessons.length === 0" class="alert alert-warning text-center small" role="alert">
            ...
            <div v-else class="table-responsive">
              <table class="table table-hover table-sm">
                ...
                <tbody>
                  <tr v-for="(lesson, index) in topic.lessons" :key="lesson.id">
                    <td>{{ index + 1 }}</td>
                    <td>{{ lesson.name }}</td>
                    <td><span class="badge bg-primary">{{ lesson.level }}</span></td>
                    <td>
                    ...
                    <td>
                      <button class="btn btn-sm btn-warning me-2" @click="editLesson(lesson.id)" title="Chỉnh sửa bài học">
                        <i class="bi bi-pencil"></i>
                      </button>
                      <button class="btn btn-sm btn-danger" @click="deleteLesson(lesson.id)" title="Xóa bài học">
                        <i class="bi bi-trash"></i>
                      </button>
                    </td>
                  </tr>
...
const emit = defineEmits(['edit-lesson', 'delete-lesson', 'edit-topic', 'delete-topic', 'refresh-data']);
...
async function fetchTopicsWithLessons() {
  ...
  try {
    ...
    const groupedTopics = topics.map(topic => {
      const lessonsForTopic = lessons.filter(lesson => lesson.topic_id === topic.id);
      return {
        ...topic,
        lessons: lessonsForTopic.sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt)) // Sort lessons by updated date
      };
