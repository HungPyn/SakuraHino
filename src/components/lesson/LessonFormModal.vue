<template>
  <div
    ...
    id="lessonFormModal"
    tabindex="-1"
    aria-labelledby="lessonFormModalLabel"
    aria-hidden="true"
    ref="modalRef"
  >
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="lessonFormModalLabel">
        ...
        <div class="modal-body">
          <form @submit.prevent="saveLesson">
            <div class="row">
              <div class="col-md-6 mb-3">
                <label for="lessonName" class="form-label"
                  >Tên Bài học <span class="text-danger">*</span></label
                >
                <input
                  type="text"
                  class="form-control"
              ...
              <div class="col-md-6 mb-3">
                <label for="lessonLevel" class="form-label"
                  >Cấp độ <span class="text-danger">*</span></label
                >
                <select
                  class="form-select"
            ...
            <div class="mb-3">
              <label for="lessonTopic" class="form-label"
              ...
              <div class="invalid-feedback" v-if="errors.topic_id">
                {{ errors.topic_id }}
              </div>
            </div>
            ...
            <div class="mb-3">
              <label for="lessonImageUrl" class="form-label"
                >URL Hình ảnh</label
              >
              <input
                type="url"
              ...
              />
              <div class="invalid-feedback" v-if="errors.imageUrl">
                {{ errors.imageUrl }}
              </div>
            </div>
            ...
            <div class="mb-4 p-3 border rounded">
              ...
              >
                <div class="col-md-4">
                  <label :for="'vocabWord' + index" class="form-label small"
                    >Từ/Cụm từ</label
                  >
                  <input
                    type="text"
                ...
                <div class="col-md-4">
                  <label :for="'vocabMeaning' + index" class="form-label small"
                    >Nghĩa</label
                  >
                  <input
                    type="text"
                    class="form-control form-control-sm"
                    :id="'vocabMeaning' + index"
                    v-model="vocab.meaning"
                  />
                </div>
                <div class="col-md-3">
                  <label :for="'vocabReading' + index" class="form-label small"
                    >Cách đọc (Hiragana/Katakana)</label
                  >
                  <input
                    type="text"
                    class="form-control form-control-sm"
                    :id="'vocabReading' + index"
                ...
                <div class="col-md-1">
                  <button
                    ...
                    @click="removeVocabularyItem(index)"
                  >
            ...
            <div class="mb-4 p-3 border rounded">
              <h5 class="d-flex justify-content-between align-items-center">
                ...
                <button
                  type="button"
                  class="btn btn-sm btn-outline-primary"
                  @click="addGrammarItem"
                >
                  <i class="bi bi-plus-circle"></i> Thêm cấu trúc
                </button>
              </h5>
              <div
                v-for="(grammar, index) in lesson.grammar"
                :key="index"
                ...
                <div class="row g-2 mb-2">
                  <div class="col-md-6">
                    ...
                    <input
                      type="text"
                      class="form-control form-control-sm"
                      :id="'grammarStructure' + index"
                      v-model="grammar.structure"
                    />
                  </div>
                  <div class="col-md-6">
                    <label
                      :for="'grammarMeaning' + index"
                      class="form-label small"
                      >Ý nghĩa</label
                    >
                    <input
                      type="text"
                      class="form-control form-control-sm"
                      :id="'grammarMeaning' + index"
                      v-model="grammar.meaning"
                    />
                  </div>
                </div>
                <div class="mb-2">
                  <label
                    :for="'grammarExplanation' + index"
                    class="form-label small"
                    >Giải thích</label
                  >
                  <textarea
                    class="form-control form-control-sm"
                    :id="'grammarExplanation' + index"
                    v-model="grammar.explanation"
                    rows="2"
                  ></textarea>
                </div>
                <div class="text-end">
            ...
            <div class="mb-4 p-3 border rounded">
              <h5>Luyện nghe</h5>
              <div class="mb-3">
                <label for="listeningTitle" class="form-label"
                  >Tiêu đề phần nghe</label
                >
                <input
                  type="text"
                  class="form-control"
                  id="listeningTitle"
                  v-model="lesson.listening.title"
                />
              </div>
              <div class="mb-3">
                ...
                <input
                  ...
                  v-model="lesson.listening.audioUrl"
                />
              </div>
              <div class="mb-3">
                ...
                <textarea
                  class="form-control"
                  id="listeningScript"
                  v-model="lesson.listening.script"
                  rows="4"
                ></textarea>
              </div>
              ...
              <div
                v-for="(q, qIndex) in lesson.listening.questions"
                :key="qIndex"
                class="mb-3 p-2 border rounded bg-light"
              >
                <div class="mb-2">
                  <label
                    :for="'listeningQuestionText' + qIndex"
                    class="form-label small"
                  ...
                  <input
                    type="text"
                    class="form-control form-control-sm"
                    :id="'listeningQuestionText' + qIndex"
                    v-model="q.question"
                  />
                </div>
                <div class="mb-2">
            ...
            <div class="mb-4 p-3 border rounded">
              ...
              <div
                v-for="(exercise, exIndex) in lesson.exercises"
                :key="exIndex"
                class="mb-3 border-bottom pb-3"
              >
                <div class="mb-2">
                  <label
                    :for="'exerciseType' + exIndex"
                    class="form-label small"
                    >Loại bài tập</label
                  >
                  <select
                    class="form-select form-select-sm"
                    :id="'exerciseType' + exIndex"
                ...
                <div class="mb-2">
                  <label
                    :for="'exerciseQuestion' + exIndex"
                    class="form-label small"
                    >Nội dung/Hướng dẫn</label
                  >
                  <textarea
                    class="form-control form-control-sm"
                    :id="'exerciseQuestion' + exIndex"
                ...
                <div
                  v-if="exercise.type === 'fill_in_blank'"
                  class="ms-3 p-2 border rounded bg-light"
                >
                  <label class="form-label small"
                    >Đáp án đúng (phân cách bởi dấu phẩy nếu nhiều từ)</label
                  >
                  <input
                    type="text"
                    class="form-control form-control-sm"
                ...
                >
                  <label class="form-label small"
                    >Các lựa chọn và đáp án đúng</label
                  >
                  <button
                    type="button"
                    class="btn btn-sm btn-secondary mb-2"
                    @click="addOption(exercise)"
                  >
                    Thêm lựa chọn
                  </button>
                  <div
                  ...
                  >
                    ...
                    <div class="form-check me-2">
                      <input
                        ...
                        type="radio"
                        :name="'mc_answer_' + exIndex"
                        :id="'mc_answer_' + exIndex + '_' + optIndex"
                        :value="option.text"
                        v-model="exercise.answer"
                      />
                      <label
                        class="form-check-label small"
                        :for="'mc_answer_' + exIndex + '_' + optIndex"
                        >Đúng</label
                      >
                    </div>
                    <button
                      ...
                      @click="removeOption(exercise, optIndex)"
                    >
                ...
                >
                  ...
                  <button
                    type="button"
                    class="btn btn-sm btn-secondary mb-2"
                    @click="addPair(exercise)"
                  >
                    Thêm cặp
                  </button>
                  <div
                    v-for="(pair, pairIndex) in exercise.pairs"
                    :key="pairIndex"
                    class="d-flex mb-1 align-items-center"
                  >
                    <input
                    ...
                    <button
                      type="button"
                      class="btn btn-sm btn-outline-danger"
                      @click="removePair(exercise, pairIndex)"
                    >
                      <i class="bi bi-dash-circle"></i>
                    </button>
                  </div>
...
const props = defineProps({
  ...
  availableLevels: {
    type: Array,
    default: () => ["N5", "N4", "N3", "N2", "N1"], // hoặc [] nếu bạn muốn dynamic
  },
});
const { availableLevels } = props;
...
async function saveLesson() {
  ...
  const validationResult = lessonService.validateLessonData(
    lesson,
...
function removeOption(exercise, index) {
  ...
  if (exercise.answer === exercise.options[index]?.text) {
    // Check if it's still the answer
    exercise.answer = "";
  }
}
