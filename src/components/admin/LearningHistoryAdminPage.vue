        ...
        <table class="history-table">
          ...
          <tbody>
            <tr v-for="h in filteredHistories" :key="h.id">
              <td>{{ h.user }}</td>
              <td>
                <span :class="['badge', getTopicClass(h.topic)]">{{ h.topic }}</span>
              </td>
              <td>
              ...
              <td class="action-buttons text-center">
                <button class="btn-action btn-edit" @click="openEditForm(h)" title="Sửa">
                  <i class="bi bi-pencil"></i>
                </button>
                <button class="btn-action btn-delete" @click="confirmDelete(h.id)" title="Xóa">
    ...
    <div v-if="showForm" class="form-modal-overlay">
      <div class="form-modal-content">
        <div class="modal-header">
          <h2 class="modal-title">{{ selectedHistory ? 'Chỉnh sửa' : 'Thêm mới' }}</h2>
...
import { getAllHistories, addHistory, updateHistory, deleteHistory } from "@/services/learningHistoryService.js";
import Swal from 'sweetalert2';

// State
const histories = ref(getAllHistories());
...
const filteredHistories = computed(() => {
  ...
  if (status) {
    ...
    result = result.filter(h => {
      ...
      return isCompleted ? progressValue === 100 : progressValue > 0 && progressValue < 100;
    });
  }
...
const saveHistory = (data) => {
  try {
    if (data.id) {
      ...
      showSuccessPopup('Cập nhật thành công!', 'Lịch sử ôn tập đã được cập nhật.');
    } else {
      addHistory(data);
      showSuccessPopup('Thêm mới thành công!', 'Lịch sử ôn tập mới đã được thêm.');
    }
