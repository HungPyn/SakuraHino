...
export const addExamResult = (newResult) => {
  const newId =
    examResults.length > 0 ? Math.max(...examResults.map((r) => r.id)) + 1 : 1;
...
export const getAllExam = async () => {
  try {
    ...
    const response = await axios.get(baseApi.main + "/api/jlpt/admin", {
      headers: {
...
export const deleteExamm = async (id) => {
  try {
    const token = localStorage.getItem("token");
    const response = await axios.post(
      `${baseApi.main}/api/jlpt/admin/delete`,
      null,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        params: { id }, // gửi id dưới dạng query param
      }
    );
...
export const addExamm = async (exam) => {
  try {
    const token = localStorage.getItem("token");
    const formData = new FormData();
    if (exam.file) formData.append("file", exam.file);
    if (exam.id) formData.append("id", exam.id);
    formData.append("examName", exam.examName);
    formData.append("status", exam.status);
    formData.append("audioUrl", exam.audioUrl || "");
    formData.append("examTime", exam.examTime || 0);
    formData.append("textReading", exam.textReading || "");

    const response = await axios.post(
      `${baseApi.main}/api/jlpt/admin`,
      formData,
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    return response.data.data; // dữ liệu trả về từ server
  } catch (error) {
